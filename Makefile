...
VER="395"
VERDOT="3.9.5"
VERDOTUNQUOTED=3.9.5
VERSUPP=
VER_SOMAJOR=3
VER_SOMINOR=9
#REG_DIR=$(PACKAGE)-$(VERDOT)
REG_DIR=regina-rexx-3.9.5
DEBIAN_DIR=$(REG_DIR)/debian
MYDISTRO=i370-ibm-openedition
MACH_ARCH=i370
...
srcdir  = .
abs_srcdir  = /u/ts8004/git/regina
...
addonsdir = /usr/local/lib/regina-rexx/addons
brewaddonsdir = 
target = i370-ibm-openedition
gcidir = $(srcdir)/gci
binarybitprefix=
packagesdir = /u/ts8004/git/regina/packages
debtopdir = 
rpmtopdir = 
apktopdir = 
...
RANLIB = :
LN_S  = ln -s
CFLAGS = -DDYNAMIC -fvisibility=default -fzos-le-char-mode=ascii -O -D_ALL_SOURCE -D_LARGE_TIME_API -D_OPEN_MSGQ_EXT -D_OPEN_SYS_FILE_EXT=1 -D_OPEN_SYS_SOCK_IPV6 -DPATH_MAX=1024 -D_UNIX03_SOURCE -D_UNIX03_THREADS  -D_UNIX03_WITHDRAWN -D_XOPEN_SOURCE=600 -D_XOPEN_SOURCE_EXTENDED -O -g
LDFLAGS = --shared

RANLIB_DYNAMIC = 
CEXTRA =    -DREGINA_SHARE_DIRECTORY=\"$(sharedir)\"  -DREGINA_VERSION_DATE=\""25 Jun 2022"\" -DREGINA_VERSION_MAJOR=\"3\" -DREGINA_VERSION_MINOR=\"9\" -DREGINA_VERSION_RELEASE=\"5\" -DREGINA_VERSION_SUPP=\"\" -DREGINA_BITS=32
DEBUG = -DNDEBUG
DEBUGGING = 
TRACEMEM = 
FLISTS = 
...
LIBPST = .a
LIBEXE = ar
LIBFLAGS = cr libregina.a
LIBLINK = -L. -lregina
LINKSHL = -L. -lregina
LINKREG = libregina.so
SHLPRE = lib
SHLPST = .so
MODPST = .so
BUNDLE = 
...
#BOTHLIBS =  $(SHLIBS)
BOTHLIBS =
REXX_BINARY_LIBS =    -lcurses   
REGINA_BINARY_LIBS = -L. -lregina
LIBREGINA_BINARY_LIBS =      
LIBREGUTIL_BINARY_LIBS = -lcurses  
THREADER_BINARY_LIBS = -L. -lregina 
EXECISER_BINARY_LIBS = -L. -lregina
STACK_BINARY_LIBS =  
SHL_NAME_REGINA  = libregina.so
SHL_NAME_REGUTIL = libregutil.so
SHL_NAME_RXTEST1 = librxtest1.so
SHL_NAME_RXTEST2 = librxtest2.so
LD_RXLIB_A1 = ibm-clang64 -shared -o $(@)
LD_RXLIB_A2 = ibm-clang64 -shared -o $(@)
LD_RXLIB_UTILA = ibm-clang64 -shared -o $(@)
LD_RXLIB_B1 = -L. -lregina
LD_RXLIB_B2 = -L. -lregina
LD_RXLIB_UTILB = -L. -lregina
DYNAMIC_LDFLAGS = 
SHL_LD = ibm-clang64 -shared -o libregina.so $(SHOFILES) -lc $(BOTHLIBS) $(MH_MT_LIBS)
SHL_TARGETS = libregina.so regina libregutil.so librxtest1.so librxtest2.so 
OTHER_INSTALLS = regina installlib 
...
OFILES = funcs.$(OBJ) builtin.$(OBJ) error.$(OBJ) variable.$(OBJ) \
	...
	wrappers.$(OBJ) $(ALLOCA) mt_notmt.$(OBJ) instore.$(OBJ) arxfuncs.$(OBJ) mygetopt.$(OBJ)
...
all : rexx$(binarybitprefix)$(EXE) $(STATICLIB) execiser$(binarybitprefix)$(EXE) $(SHL_TARGETS) rxstack$(binarybitprefix)$(EXE) rxqueue$(binarybitprefix)$(EXE) $(MTBFILES)
...
rxstack$(binarybitprefix)$(EXE) : rxstack.$(OBJ) erexxbif.$(OBJ) eextstack.$(OBJ) mygetopt.$(OBJ) LibSha1.$(OBJ)
	$(PURIFY) $(CC) $(LINKOPT) $(LDFLAGS) $(DYNAMIC_LDFLAGS) -o $@ rxstack.$(OBJ) erexxbif.$(OBJ) eextstack.$(OBJ) mygetopt.$(OBJ) LibSha1.$(OBJ) $(STACK_BINARY_LIBS)
	$(LDEXTRA)

rxqueue$(binarybitprefix)$(EXE) : rxqueue.$(OBJ) erexxbif.$(OBJ) eextstack.$(OBJ) mygetopt.$(OBJ)
	$(PURIFY) $(CC) $(LINKOPT) $(LDFLAGS) $(DYNAMIC_LDFLAGS) -o $@ rxqueue.$(OBJ) erexxbif.$(OBJ) eextstack.$(OBJ) mygetopt.$(OBJ) $(STACK_BINARY_LIBS)
	$(LDEXTRA)
...
rpm: bzip $(rpmtopdir)
	...
	rm -f $(rpmtopdir)/RPMS/*/libregina3-devel-$(VERDOTUNQUOTED)*.rpm
	rm -f $(rpmtopdir)/RPMS/*/libregina3-$(VERDOTUNQUOTED)*.rpm
	rpmbuild -ba --clean --define "initservice installsysvinit" --define "systemdinstallpath " --define "_topdir " ./Regina.spec
	...
	mv $(rpmtopdir)/RPMS/*/libregina3-devel-$(VERDOTUNQUOTED)-[0-9]*.rpm `dirname $(rpmtopdir)/RPMS/*/libregina3-devel-$(VERDOTUNQUOTED)-[0-9]*.rpm`/`basename $(rpmtopdir)/RPMS/*/libregina3-devel-$(VERDOTUNQUOTED)-[0-9]*.rpm .rpm`-$(MYDISTRO).rpm
	mv $(rpmtopdir)/RPMS/*/libregina3-$(VERDOTUNQUOTED)-[0-9]*.rpm `dirname $(rpmtopdir)/RPMS/*/libregina3-$(VERDOTUNQUOTED)-[0-9]*.rpm`/`basename $(rpmtopdir)/RPMS/*/libregina3-$(VERDOTUNQUOTED)-[0-9]*.rpm .rpm`-$(MYDISTRO).rpm
...
rpminstall-devel:
	sudo rpm -Uv $(rpmtopdir)/RPMS/*/libregina3-$(VERDOTUNQUOTED)-[0-9]*-$(MYDISTRO).rpm
	sudo rpm -Uv $(rpmtopdir)/RPMS/*/$(PACKAGE)-$(VERDOTUNQUOTED)-[0-9]*-$(MYDISTRO).rpm
	sudo rpm -Uv $(rpmtopdir)/RPMS/*/libregina3-devel-$(VERDOTUNQUOTED)-[0-9]*-$(MYDISTRO).rpm
...
install-rexx: rexx$(EXE) regina$(EXE) installsysvinit-deb
	$(INSTALL) -m 755 -d $(DESTDIR)$(libdir)
	$(INSTALL) -m 755 -d $(DESTDIR)$(addonsdir)
	$(INSTALL) -m 755 -d $(DESTDIR)$(sysconfdir)
...
installsystemd-deb:
	-$(INSTALL) -m 755 -c ./rxstack.service $(DESTDIR)/rxstack.service
...
# of the one generated by the following configure
	...
	(cd $(packagesdir)/HPKG/build/$(PACKAGE);package create ../../regina_rexx-3.9.5-1-i370.hpkg;)
	(cd $(packagesdir)/HPKG/build/$(PACKAGE)_devel;package create ../../libregina3_devel-3.9.5-1-i370.hpkg;)
	(cd $(packagesdir)/HPKG/build/$(PACKAGE)_lib;package create ../../libregina3-3.9.5-1-i370.hpkg;)
